...
outputX = []
outputX.append("X" + "simple_tallchair")
outputY = []
outputY.append("Y" + "simple_tallchair")
outputZ = []
outputZ.append("Z" + "simple_tallchair")
    
mesh = trimesh.load('recognition_models/final_mods/simple_tallchair.obj')
bounds = mesh.bounds

# get all values in space between the upper and lower bounds of the model
cutArr_x = np.linspace(bounds[0][0], bounds[1][0], 20)
cutArr_y = np.linspace(bounds[0][1], bounds[1][1], 20)
cutArr_z = np.linspace(bounds[0][2], bounds[1][2], 20)
